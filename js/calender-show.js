const holidays = [
  ...
  20231003, 20231009, 20231225, 20240101, 20240209, 20240210, 20240211,
  20240212, 20240301, 20240505, 20240506, 20240515, 20240606, 20240815,
  20240916, 20240917, 20240918, 20241003, 20241009, 20241225, 20250101,
  20250128, 20250129, 20250130, 20250301, 20250303, 20250505, 20250506,
  20250508, 20250606, 20250815, 20251003, 20251005, 20251006, 20251007,
  20251008, 20251009, 20251225, 20260101, 20260216, 20260217, 20260218,
  20260301, 20260505, 20260508, 20260606, 20260815, 20260924, 20260925,
  20260926, 20260927, 20261003, 20261009, 20261225, 20260302, 20260525,
  20260817, 20261005, 20270101, 20270206, 20270207, 20270208, 20270209,
  20270301, 20270303, 20270505, 20270513, 20270606, 20270815, 20270816,
  20270914, 20270915, 20270916, 20271003, 20271004, 20271009, 20271011,
  20271225, 20280101, 20280126, 20280127, 20280128, 20280301, 20280502,
  20280505, 20280606, 20280815, 20281002, 20281003, 20281004, 20281005,
  20281009, 20281225, 20290101, 20290212, 20290213, 20290214, 20290301,
  20290505, 20290507, 20290520, 20290606, 20290815, 20290921, 20290922,
  20290923, 20290924, 20291003, 20291009, 20291225,
];
...
function makeCalendar() {
  ...
  while (true) {
    // 현재 월이 마지막 월이고 count(현재 일)가 endDay+1과 같아지면 종료됨
    htmlDummy += `<div id='M${currentMonth}day${count}'>${currentMonth}월 ${count}일</div>`;
    count++;

    if (count > endDay && currentMonth === endMonth) {
      break;
    }
...
function makeStudentsObj() {
  // 학생들의 이름, 청소 불가능 요일, 청소 횟수 등을 담은 객체배열
...
function cleaningNumSort(arr) {
  // 청소 횟수로 객체배열을 정렬
  arr.sort((a, b) => a.cleaning_num - b.cleaning_num);
}
...
function getZero(Num) {
  // hoilday와 비교를 위해 10 아래 숫자들은 앞에 0을 붙여줌
  if (Num < 10) {
    newNum = '0' + String(Num);
    return newNum;
  } else {
...
function noCleaningPerson() {
  ...
  if (localStorage.getItem('checkArr') != null) {
    for (let i = 0; i < studentName.length; i++) {
      for (let j = 1; j <= 5; j++) {
        document
          .getElementById(`studentcheck${i}`)
          .querySelector(`#checkbox${j}`).checked = getCheckArr[i][j];
      }
    }
  }
}
...
function makeCleaningSchedule() {
  ...
  while (true) {
    ...
    currentDate = parseInt(
      String(startYear) + getZero(currentMonth) + getZero(count)
    );
    if (currentDay != 0 && currentDay != 6 && !holidays.includes(currentDate)) {
      ...
      document.querySelector(
        `#M${currentMonth}day${count}`
      ).innerHTML += `<br>${studentsObj[index].name}`;
      ...
      index = 0;
      while (
        compare == studentsObj[index] ||
        studentsObj[index]['available_day'][currentDay - 1]
      ) {
        // 한명이 하루에 두번배정되지 않도록 가능한 사람을 찾는다.
        index++;
        if (index == studentsObj.length) {
          return noCleaningPerson();
        }
      ...
      document
        .querySelector(`#M${currentMonth}day${count}`)
        .classList.add('blue');
    } else {
      document
        .querySelector(`#M${currentMonth}day${count}`)
